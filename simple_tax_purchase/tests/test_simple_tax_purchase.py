...
class TestSimpleTaxPurchase(TransactionCase):
    """Tests for 'Simple Tax - Purchase' Module"""

    def setUp(self):
        super(TestSimpleTaxPurchase, self).setUp()
        self.order = self.env.ref('simple_tax_purchase.purchase_order_1')
        ...
        self.order_line_tax_incl = self.env.ref(
            'simple_tax_purchase.purchase_order_1_line_1')
        self.order_line_tax_excl = self.env.ref(
            'simple_tax_purchase.purchase_order_1_line_2')
    ...
    def _check_tax_line(self, line, tax):
        self.assertEqual(
            len(line.taxes_id), 1,
            "Incorrect quantity of taxes on the purchase line.")
        self.assertEqual(
            line.taxes_id[0].id, tax.id,
            "Incorrect tax on the purchase line.")
