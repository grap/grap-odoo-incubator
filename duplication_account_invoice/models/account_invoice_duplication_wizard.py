...
class AccountInvoiceDuplicationWizard(models.TransientModel):
    ...
    begin_date = fields.Date(
        string="Begin Date", required=True, default=lambda s: s._default_begin_date()
    )

    include_current_date = fields.Boolean(string="Include Current Date", default=False)
        ...
        if self.begin_date and self.duplication_type and self.duplication_duration:
            ...
            for i in range(begin_index, end_index):
                if self.duplication_type == "week":
                    date_invoice = self.begin_date + datetime.timedelta(weeks=i)
                else:
                    date_invoice = self.begin_date + relativedelta(months=i)
                date_due = date_invoice + datetime.timedelta(
