...
class InternalUse(models.Model):
    ...
    description = fields.Char(
        string="Description",
        states={"done": [("readonly", True)], "confirmed": [("readonly", True)]},
    )
    ...
    company_id = fields.Many2one(
        ...
        string="Company",
        readonly=True,
        related="internal_use_case_id.company_id",
        store=True,
    )

    state = fields.Selection(
        selection=_INTERNAL_USE_STATE, string="Status", readonly=True, default="draft"
    )

    line_ids = fields.One2many(
        comodel_name="internal.use.line",
        inverse_name="internal_use_id",
        string="Lines",
        states={"done": [("readonly", True)], "confirmed": [("readonly", True)]},
    )
    ...
    stock_move_qty = fields.Integer(
        string="Stock Move Quantity", compute="_compute_stock_move_qty", store=True
    )

    account_move_id = fields.Many2one(
        comodel_name="account.move", string="Account Moves", readonly=True, copy=False
    )

    amount = fields.Float(
        string="Total Amount (Tax excluded)", compute="_compute_amount", store=True
    )
    ...
    def action_confirm(self):
        ...
        for use in self.filtered(lambda x: x.state == "draft"):
            if len(use.line_ids) == 0:
                raise UserError(_("You can not confirm an empty Internal Use."))

            # Retrieve lines and create one stock move
            vals_list = []
            for line in use.line_ids:
                use_case = line.internal_use_id.internal_use_case_id
            ...
            # Name internal use
            sq_internal_use = self.env["ir.sequence"].next_by_code("internal.use")
    ...
    def action_done(self):
        ...
        for use in self.filtered(
            lambda x: (x.state == "confirmed" and x.internal_use_case_id.journal_id)
        ):
