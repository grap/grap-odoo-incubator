...
class InternalUseLine(models.Model):
    ...
    internal_use_id = fields.Many2one(
        comodel_name="internal.use",
        string="Internal Uses",
        index=True,
        readonly=True,
        ondelete="cascade",
    )
    ...
    price_unit = fields.Float(
        string="Unit Price (Tax Excluded)", digits=dp.get_precision("Product Price")
    )
    ...
    def _compute_amount(self):
        for line in self:
            ...
            line.amount = line.price_unit * line.product_uom_id._compute_quantity(
    ...
    def _on_change_product_id(self):
        ...
        line.price_unit = line.product_id.supplier_taxes_id.compute_all(
            line.price_unit, None, line.product_qty, line.product_id.id
        )["total_excluded"]
    ...
    def _prepare_stock_move(self):
        ...
        return {
            ...
            "product_uom_qty": (
                self.product_qty > 0 and self.product_qty or -self.product_qty
            ),
            "location_id": (
                self.product_qty > 0
