...
class TestStockInternalUseOfProducts(TransactionCase):
    ...
    def setUp(self):
        ...
        self.internal_use = self.env.ref("stock_internal_use_of_products.internal_use")
    ...
    def test_01_stock_move_without_account_move(self):
        ...
        self.assertEqual(
            self.internal_use_without.state,
            "done",
    ...
    def test_02_stock_move_and_account_move(self):
        ...
        self.assertEqual(
            len(self.internal_use.stock_move_ids),
            3,
        ...
        self.assertEqual(
            len(dozen_moves), 1, "Each use line should generate one unique move line"
        )
        self.assertEqual(
            dozen_moves[0].product_qty,
            3 * 12,
            "Stock move quantity should be expressed in the product UoM",
        )

        self.assertEqual(
            self.internal_use.state,
            "confirmed",
        ...
        self.assertNotEqual(
            self.internal_use.account_move_id,
            False,
            "Finish an Internal use should generate an accounting entry",
        )
        self.assertEqual(
            len(self.internal_use.account_move_id.line_ids),
            4,
        ...
        self.assertEqual(len(lines), 2, "Incorrect uncharge account move line.")
